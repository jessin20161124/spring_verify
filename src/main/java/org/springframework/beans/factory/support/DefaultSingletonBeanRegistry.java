...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	// 注册单实例的bean到singletonObjects中，这里缓存不允许覆盖已经有的key，会抛出非法状态异常。
	// 注册的时候已经初始化了
	// 在哪调用这个方法？
	// 更多的是手动注册，而非定义在xml中
	@Override
	public void registerSingleton(String beanName, Object singletonObject) throws IllegalStateException {
		...
		// singletonObjects是ConcurrentHashMap，但是还是要使用synchronized
	...
	/**
	 * 每个bean新建后都会添加到singletonFactory中，提前暴露，这里也有注册功能
	 * @param beanName
		...
		if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
			...
			synchronized (this.singletonObjects) {
				...
				if (singletonObject == null && allowEarlyReference) {
					// 2. 提前暴露的bean放在singletonFactories中
	...
	/**
	 * --这里先查询singletonObjects是否有bean存在，没有则进行创建ObjectFactory#getObject，并注册到singletonObjects中
	 * Return the (raw) singleton object registered under the given name,
	 * creating and registering a new one if none registered yet.
	 * @param beanName the name of the bean
	 * @param singletonFactory the ObjectFactory to lazily create the singleton
	 * with, if necessary --ObjectFactory是为了懒加载，懒构建？？
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) {
				// 碰到不合法情况，直接抛出自定义异常，自定义异常的类名清晰地说明了异常的类型
