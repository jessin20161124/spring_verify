...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		if (requiredType != null && bean != null && !requiredType.isAssignableFrom(bean.getClass())) {
			try {
				...
				logger.info("Failed to convert bean '" + name + "' to required type [" +
	...
	public Class<?> getType(String name) throws NoSuchBeanDefinitionException {
		...
		Object beanInstance = getSingleton(beanName, false);
		if (beanInstance != null) {
			// 不是以&开头的FactoryBean，获取的是FactoryBean#getObjectType
	...
	/**
	 * 这个类比较全了，使用了conversionService和PropertyEditor
	 * @return
	 */
	@Override
	...
	/**
	 * 使用所有的解析策略按照加入的先后顺序进行解析，如果某一个返回null，直接返回null
	...
	/**
	 * 单例可以缓存，非单例则不可以缓存，检查的是BeanDefinition
	...
	/**
	 * 子类可以覆盖，使用DisposableBeanAdapter
 	 */
	...
	protected String transformedBeanName(String name) {
		// BeanFactoryUtils是一个抽象类，但是都是静态方法，包含了多个类需要用到的工具，降低重复
	...
	protected void initBeanWrapper(BeanWrapper bw) {
		// 把conversionService和PropertyEditor都注册到bw了
	...
	protected Object getObjectForBeanInstance(
		...
		// 获取FactoryBean的方法：通过&name获得。
		// Now we have the bean instance, which may be a normal bean or a FactoryBean.
		// If it's a FactoryBean, we use it to create a bean instance, unless the
		// caller actually wants a reference to the factory.
		// 不是FactoryBean，或者是，且获取的就是FactoryBean
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
