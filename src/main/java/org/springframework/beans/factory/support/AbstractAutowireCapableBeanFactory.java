...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final Object[] args) {
		...
		try {
			...
			populateBean(beanName, mbd, instanceWrapper);
				logger.info(beanName + "设置值结束：" + instanceWrapper.getWrappedInstance());
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				logger.info("确定bean的类型：" + beanName + " class : " + targetType.getSimpleName());
				if (targetType != null) {
	...
	protected Object applyBeanPostProcessorsBeforeInstantiation(Class<?> beanClass, String beanName)
		...
		for (BeanPostProcessor bp : getBeanPostProcessors()) {
			if (bp instanceof InstantiationAwareBeanPostProcessor) {
				...
				logger.info("调用实例化bean后处理器的前实例化方法：" + ibp.getClass().getSimpleName());
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, Object[] args) {
		logger.info("开始创建beanName=" + beanName);
		// Make sure bean class is actually resolved at this point.
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, BeanWrapper bw) {
		...
		logger.info(beanName + "已经含有属性：" + pvs);
		if (bw == null) {
			if (!pvs.isEmpty()) {
			...
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					...
					if (!ibp.postProcessAfterInstantiation(bw.getWrappedInstance(), beanName)) {
						logger.info("调用实例化bean[" + beanName + "]后处理器的后实例化方法，" + ibp + "，不初始化值了");
		...
		logger.info(beanName + "获得所有属性值" + pvs);
		...
		if (hasInstAwareBpps || needsDepCheck) {
			...
			if (hasInstAwareBpps) {
				for (BeanPostProcessor bp : getBeanPostProcessors()) {
					if (bp instanceof InstantiationAwareBeanPostProcessor) {
						InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
						logger.info(beanName + "处理完pw后，调用实例化后处理器的postProcessPropertyValues方法，" + ibp);
		...
		logger.info("调用" + beanName + "，开始设置属性值" + pvs.isEmpty());
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		logger.info("设置完属性值了：" + bw.getWrappedInstance());
	}
	...
	protected Object initializeBean(final String beanName, final Object bean, RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {
			...
			logger.info("调用aware方法");
			invokeAwareMethods(beanName, bean);
		}

		Object wrappedBean = bean;
		if (mbd == null || !mbd.isSynthetic()) {
			logger.info("调用后处理器before init 方法。。");
		...
		if (mbd == null || !mbd.isSynthetic()) {
			logger.info("调用后处理器after init 方法");
