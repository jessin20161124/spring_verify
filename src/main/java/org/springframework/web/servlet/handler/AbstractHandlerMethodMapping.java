...
public abstract class AbstractHandlerMethodMapping<T> extends AbstractHandlerMapping implements InitializingBean {
	...
	protected void initHandlerMethods() {
		...
		// beanNamesForType只返回当前beanFactory注入的bean，不考虑父beanFactory的bean
		logger.info("在祖先获取处理器方法，" + detectHandlerMethodsInAncestorContexts + "，beanNames：" + Arrays.asList(beanNames));
	...
	protected void detectHandlerMethods(final Object handler) {
		...
		for (Map.Entry<Method, T> entry : methods.entrySet()) {
			logger.info("注册处理器映射关系，处理器：" + handler +
					"，方法：" + entry.getKey() + "，映射值：" + entry.getValue());
	...
	protected HandlerMethod getHandlerInternal(HttpServletRequest request) throws Exception {
		...
		try {
			...
			logger.info("查询访问路径的处理器方法 " + lookupPath + "，结果为：" + handlerMethod);
	...
	protected HandlerMethod lookupHandlerMethod(String lookupPath, HttpServletRequest request) throws Exception {
		...
		if (!matches.isEmpty()) {
			...
			logger.info("查找到 " + matches.size() + "个匹配的映射： [" +
						lookupPath + "] : " + matches);
	...
	class MappingRegistry {
		...
		// todo 主要的属性，根据RequestMappingInfo得到对应的HandlerMethod
		...
		public void register(T mapping, Object handler, Method method) {
			...
			try {
				...
				logger.info("Mapped \"" + mapping + "\" onto [" + handlerMethod.getBean() + ":" + handlerMethod.getMethod().getName() + "]");
